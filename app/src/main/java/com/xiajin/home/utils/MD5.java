...
public class MD5 {
	...
	private String getMD5ofStr(String s) {// 返回MD5串；
		int i;
		md5Init();
	...
	private void md5Update(byte abyte0[], int i) {// 补位操作，abyte0为需要进行MD5加密的字符串，i为字符串长度ￄ1�7
	...
	private void md5Final() {// ￄ1�7��处理，将得到ￄ1�728位（16字节）MD5码存放在digest数组ￄ1�7
	...
	private void Encode(byte abyte0[], long al[], int i) {// 转换函数，将al中long型的变量输出到byte型的数组abyte0中，
		// 低位字节在前，高位字节在后；
		int j = 0;
		for (int k = 0; k < i; k += 4) {
	...
	private static String byteHEX(byte byte0) {// 字节到十六进制的ASCII码转ￄ1�7
	...
	public static String toMD5(String s) {
		return new MD5().getMD5ofStr(s);
	}

	 public final static String toMD52(String s) {
